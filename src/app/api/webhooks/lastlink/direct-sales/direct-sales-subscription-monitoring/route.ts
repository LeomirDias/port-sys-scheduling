...
const LASTLINK_WEBHOOK_SECRET_SUBSCRIPTIONS_DIRECT_SALES = process.env.LASTLINK_WEBHOOK_SECRET_SUBSCRIPTIONS_DIRECT_SALES!;

export async function POST(req: NextRequest) {
    // pegar token do header
    const headerSecret = req.headers.get("x-lastlink-token");

    if (!headerSecret || headerSecret !== LASTLINK_WEBHOOK_SECRET_SUBSCRIPTIONS_DIRECT_SALES) {
